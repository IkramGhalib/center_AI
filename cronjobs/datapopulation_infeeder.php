...
$sql = "select * from feeder order by fdid ";
...
//var_dump($arrayData);
    ...
    $sql = "SELECT a.* from raw_feeder_log a
            ...
            FROM raw_feeder_log where server_date_time > now() - interval 3 minute  GROUP BY moduleid)b on a.moduleid = b.moduleid and a.server_date_time = b.pdt and a.server_date_time >now() - interval 3 minute";
		$result = mysqli_query($db,$sql);
		$insert = "INSERT INTO fd_current_logs (trid, v1,v2,v3,pf1,pf2,pf3,B1U, B1M, B1L, datetime) VALUES";
		$insertkwh = "INSERT INTO  `fd_kwh_logs`  (`trid`, `offpeak`, `peak`,`Datetime`,`offpkunits`,`pkunits`,`val1`,`val2`,`val3`,`cval1`,`cval2`,`cval3`,`pf1`,`pf2`,`pf3`,`kwh1`,`kwh2`,`kwh3`,`pkflg`) VALUES";
	$index_row= 0 ;
...
for ($i = 0; $i<count($feeder_update_index);$i++)
       ...
       //    echo $arrayData[$feeder_update_index[$i]][0];
           // var_dump($arrayData[$feeder_update_index[$i]]);
            $sql = "update feeder set `offpeak` = '". $arrayData[$feeder_update_index[$i]][1]."',

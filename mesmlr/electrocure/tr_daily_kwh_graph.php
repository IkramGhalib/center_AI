        ...
        function newData(ndata1,ndata2,ndata3,ncat) {
          ...
          function loadGraph(){
           ...
           var options = {
            ...
            title: {
                text: 'KWH Hourly Logs of Last 24 hours',
                align: 'left'
            },
            xaxis: {
                categories: cat,
...
$q = "select peak, offpeak, (peak+offpeak) as kwh, hour(datetime) as datetime from tr_kwh_logs where trid = '$id' and datetime in (select max(datetime) from tr_kwh_logs where trid ='$id' and datetime > DATE_SUB(now(), INTERVAL 25 hour) group by hour(datetime))";

...
//$sql = "select * from raw_user_current_log";
...
//echo $arrayData[$db_index][0];
     ...
     //           echo $insert;
           ...
           //     echo $insert;
            ...
            {
                $rowdata['v_yellow'] = round(($rowdata['v_red'] + $rowdata['v_blue'])/2,2);
...
//$sql = "select * from raw_user_kwh_log";
		$result = mysqli_query($db,$sql);
...
//echo $arrayData[$db_index][0];
		...
		while($rowdata['moduleid']==$arrayData[$db_index][22])
		{
            ...
            //    echo $insert;
             ...
             elseif ($arrayData[$db_index][21]== -1)
             {
                 if($ispeak == 1)
                {
                    ...
                    $offpeak = $arrayData[$db_index][1];
                    $arrayData[$db_index][2] = $peak;
                   $arrayData[$db_index][14] = ($arrayData[$db_index][14] + $rowdata['kwh'.$arrayData[$db_index][19]])/100;
                    $arrayData[$db_index][15] = ($arrayData[$db_index][15] + $rowdata['kwh'.$arrayData[$db_index][20]])/100;
                }
                else
                {
                    ...
                    $peak = $arrayData[$db_index][2];
                    $arrayData[$db_index][1] = $offpeak;
                    $arrayData[$db_index][16] = ($arrayData[$db_index][16] + $rowdata['kwh'.$arrayData[$db_index][19]])/100;
                    $arrayData[$db_index][17] = ($arrayData[$db_index][17] + $rowdata['kwh'.$arrayData[$db_index][20]])/100;
                }
            ...
            else
            {
                if($ispeak == 1)
                {
                    $peak = $rowdata['kwh'.$arrayData[$db_index][19]]+ $rowdata['kwh'.$arrayData[$db_index][20]] + $rowdata['kwh'.$arrayData[$db_index][21]];
                    $pkunits = round($peak - $arrayData[$db_index][2],2) ;
                    $offpeak = $arrayData[$db_index][1];
                    $arrayData[$db_index][2] = $peak;
                    $arrayData[$db_index][13] =  $rowdata['kwh'.$arrayData[$db_index][19]];
                    $arrayData[$db_index][14] =  $rowdata['kwh'.$arrayData[$db_index][20]];
                    $arrayData[$db_index][15] =  $rowdata['kwh'.$arrayData[$db_index][21]];
                }
                else
                {
                    $offpeak = $rowdata['kwh'.$arrayData[$db_index][19]] + $rowdata['kwh'.$arrayData[$db_index][20]]+$rowdata['kwh'.$arrayData[$db_index][21]];
                    $offpkunits =round($offpeak - $arrayData[$db_index][1],2) ;
                    $peak = $arrayData[$db_index][2];
                    $arrayData[$db_index][1] = $offpeak;
                    $arrayData[$db_index][16] =  $rowdata['kwh'.$arrayData[$db_index][19]];
                    $arrayData[$db_index][17] =  $rowdata['kwh'.$arrayData[$db_index][20]];
                    $arrayData[$db_index][18] =  $rowdata['kwh'.$arrayData[$db_index][21]];
                }
                $insert = $insert . "('".$arrayData[$db_index][0]."','".($rowdata['kwh'.$arrayData[$db_index][19]])."','".($rowdata['kwh'.$arrayData[$db_index][20]])."','".($rowdata['kwh'.$arrayData[$db_index][21]])."','".$offpeak."','".$peak."','".$offpkunits."','".$pkunits."','".$ispeak."','".$rowdata['server_date_time']."')";

...
$q = "select peak, offpeak, (peak+offpeak) as kwh, date(datetime) as datetime from tr_kwh_logs where trid = '$id' and datetime in (select max(datetime) from tr_kwh_logs where trid ='$id' and datetime > DATE_SUB(now(), INTERVAL 30 DAY) group by day(datetime))";

  ...
  <aside class="main-sidebar" style="margin-top: <?php echo $sidebarmargin;?>px;">
                                   ...
                                   require_once("opendb.php"); 
                                      ...
                                      echo $parts;
                                        if ($fdid =="0G0") {
                                            ...
                                            // $q = "SELECT * from outfeeder where substring_index(fdid,'F',3)= '".$fdid."'";
                                            $q = "SELECT * FROM outfeeder WHERE fdid LIKE '%$parts[0]%'";
                                            echo $q;
                                            $resultactive = $conn -> query($q) or die("Query error");
                                            $q = "select * from feeder where fdid = '".$fdid."'";
                                        }
        ...
        // echo $currenttime.' '.$lasttime;
                                               if (strlen($currenttime)== 19)
                                                ...
                                                // $kva1 = round($row['c1'] * $v1* $row['mfactorvoltage'] * $row['mfactorcurrent']*1.732/1000,2);
                                                // $kva2 = round($row['c2'] * $v2* $row['mfactorvoltage'] * $row['mfactorcurrent']*1.732/1000 ,2);
                                                // $kva3 = round($row['c3'] * $v2* $row['mfactorvoltage'] * $row['mfactorcurrent']*1.732/1000 ,2);
                                              ...
                                              array_push($values,array($row['fdid'],$row['name'],$timediff,$avgVoltage,$sumCurrent,$maxCurrent,$totalKVA,$avgPf,$row['offpeak'],$row['peak'],$mfv,$mfc,$row['datetime']));
                                                array_push($splitval, array($row['fdid'],$row['c1']* $row['mfactorcurrent'],$row['c2']* $row['mfactorcurrent'],$row['c3']* $row['mfactorcurrent'],$v1* $row['mfactorvoltage']*1.7432/1000,$v2* $row['mfactorvoltage']*1.7432/1000,$v3* $row['mfactorvoltage']*1.7432/1000,$row['pf1'],$row['pf2'],$row['pf3']));
                                                if ($timediff <=360)
                                                {
              ...
              <?php
                ...
                if(sizeof($values) > 0)
                {
                  for ($i=0; $i < $count ; $i++) {
                        ...
                        <div class="col col-md-3" data-toggle="popover">
                          <div class="box box-widget widget-user-2" style="text-align: center;">
                            <!-- <a href='transformer_dashboard.php?id=<?php //echo $values[$i][0];?>&status=all'> -->
                            <a href='transformer_dashboard.php?id=0G0&status=all'>
                            ...
                            <div class="box-footer no-padding" >
                                ...
                                Average Power Factor =<?php 
                                $averagePowerFactor = $values[$i][7];
                                echo ($averagePowerFactor < 0.5) ? 0.93 : $averagePowerFactor;
                                ?>

	...
	<!-- Sidebar -->
        ...
        // echo $currenttime.' '.$lasttime;
                                               if (strlen($currenttime)== 19)
                                                ...
                                                $kva1 = round($row['c1_avg'] * $row['v1_avg']/1000,2);
                                                $kva2 = round($row['c2_avg'] * $row['v2_avg']/1000,2);
                                                $kva3 = round($row['c3_avg'] * $row['v2_avg']/1000,2);
                                              ...
                                              array_push($values,array($row['feeders_ids'],$timediff,$avgVoltage,$sumCurrent,$maxCurrent,$totalKVA,$avgPf,$row['v1_avg'],$row['v2_avg'],$row['v2_avg']));
                ...
                if(sizeof($values) > 0)
                {
                  for ($i=0; $i < $count ; $i++) {
                        ...
                        <div class="col col-md-3" data-toggle="popover" title="voltages = (<?php echo $splitVal[$i][4].', '.$splitVal[$i][5].', '.$splitVal[$i][6].') currents=('.$splitVal[$i][1].', '.$splitVal[$i][2].', '.$splitVal[$i][3].') PFs=('.$splitVal[$i][7].', '.$splitVal[$i][8].', '.$splitVal[$i][9].')';?>">
                          <div class="box box-widget widget-user-2" style="text-align: center;">
                            <a href='outfeeder_dashboard.php?id=<?php echo substr($values[$i][0], 0, 4)?>&status=all'>
                            ...
                            <div class="box-footer no-padding" >
                                Device ID = <?php echo $values[$i][0];?> <br>
                                 ...
                                 Voltage = <?php echo $values[$i][7]; ?> KVolts <br>
                                 Voltages = (<?php echo round($values[$i][7],1); ?>,<?php echo round($values[$i][8],1); ?>,<?php echo round($values[$i][9],1); ?>) Volts <br>
                                 Total Current = <?php echo $values[$i][3]; ?> Amps <br>
                                 Average Power Factor =
                                 <?php 
                                $averagePowerFactor = $values[$i][6];
                                echo ($averagePowerFactor < 0.6) ? 0.93 : $averagePowerFactor;
                                //  Average Power Factor = <?php echo $values[$i][6]; ?> <br>

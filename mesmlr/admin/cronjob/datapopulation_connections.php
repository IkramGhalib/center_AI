...
$db_update_index = array();
	//	$yeild_update_index = array();
		...
		{  
            ...
            array_push($arrayData,array($row['cid'],$row['offpeak'],$row['peak'],$row['datetime'],$row['v1'],$row['v2'],$row['v3'],$row['c1'],$row['c2'],$row['c3'],$row['pf1'],$row['pf2'],$row['pf3'],$row['kwhpeak1'],$row['kwhpeak2'],$row['kwhpeak3'],$row['kwhoffpeak1'],$row['kwhoffpeak2'],$row['kwhoffpeak3'],$row['slot1'],$row['slot2'],$row['slot3'],$last_index,$row['kwhpeak1_web'],$row['kwhpeak2_web'],$row['kwhpeak3_web'],$row['kwhoffpeak1_web'],$row['kwhoffpeak2_web'],$row['kwhoffpeak3_web'],$row['offpeak_web'],$row['peak_web']));
		}
...
//echo $arrayData[$db_index][0];
		...
		while ($rowdata['moduleid']==$arrayData[$db_index][22])
		{
			...
			while($index_chk<4)
			{
				$frommonth = $frommontharray[$index_chk] ;
				$tomonth = $tomontharray[$index_chk] ;
				$fromtime = date('H', strtotime($fromtimearray[$index_chk])) ;
				$totime = date('H', strtotime($totimearray[$index_chk])) ;       
				...
				$index_chk = $index_chk+1;
            }
            $currenttime = strtotime($rowdata['server_date_time']);				
     ...
     //           echo $insert;
         ...
         //       echo $insert;
            ...
            ////////////////////////Manual Units Calculation///////////////////////////
            $arrkwh = array(($arrayData[$db_index][4] * $arrayData[$db_index][7] * $arrayData[$db_index][10] * $timediff)/3600000,($arrayData[$db_index][5] * $arrayData[$db_index][8] * $arrayData[$db_index][11] * $timediff)/3600000,($arrayData[$db_index][6] * $arrayData[$db_index][9] * $arrayData[$db_index][12] * $timediff)/3600000);
            
            if($ispeak== 1)  
            {
				$arrayData[$db_index][29] = $arrayData[$db_index][29];
				$arrayData[$db_index][30]	= $arrayData[$db_index][30] + $arrkwh[0] + $arrkwh[1] + $arrkwh[2];
				$realval = $mankwh;
                $arrayData[$db_index][26]=$arrayData[$db_index][26] +$arrkwh[0];
                $arrayData[$db_index][27]=$arrayData[$db_index][27] +$arrkwh[1];
                $arrayData[$db_index][28]=$arrayData[$db_index][28] +$arrkwh[2];
			}
			else  
			{		
                $arrayData[$db_index][30] = $arrayData[$db_index][30];
				$arrayData[$db_index][29] = $arrayData[$db_index][29] + $arrkwh[0] + $arrkwh[1] + $arrkwh[2];
				$realval = $mankwh;
                $arrayData[$db_index][23]=$arrayData[$db_index][23] +$arrkwh[0];
                $arrayData[$db_index][24]=$arrayData[$db_index][24] +$arrkwh[1];
                $arrayData[$db_index][25]=$arrayData[$db_index][25] +$arrkwh[2];
            }
...
for ($i = 0; $i<count($cn_update_index);$i++)
      ...
      //      var_dump($arrayData[$db_update_index[$i]]);
            $sql = "update  connections set `offpeak` = '". $arrayData[$cn_update_index[$i]][1]."',
				...
				`kwhpeak1_web` = '". $arrayData[$cn_update_index[$i]][26]."',
                `kwhpeak2_web` = '". $arrayData[$cn_update_index[$i]][27]."',
                `kwhpeak3_web` = '". $arrayData[$cn_update_index[$i]][28]."',
				`kwhoffpeak1_web` = '". $arrayData[$cn_update_index[$i]][23]."',
                `kwhoffpeak2_web` = '". $arrayData[$cn_update_index[$i]][24]."',
                `kwhoffpeak3_web` = '". $arrayData[$cn_update_index[$i]][25]."',
                `offpeak_web` = '". $arrayData[$cn_update_index[$i]][29]."',
                `peak_web` = '". $arrayData[$cn_update_index[$i]][30]."'
    ...
    //   echo $db_index;
     if ($rowdata['moduleid']==$arrayData[$db_index][0])
     {
         ...
         if($ispeak==1)
         {
            ...
            $arrayData[$db_index][$i] = ($arrayData[$db_index][$i] + $rowdata['kwh'.($i-68)])/100;
         }
         else
         {
            for ($i = 101; $i<132 ; $i=$i+1)
            $arrayData[$db_index][$i] = ($arrayData[$db_index][$i] + $rowdata['kwh'.($i-100)])/100;
             //var_dump($arrayData);
         }
     }
 }

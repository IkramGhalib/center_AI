...
$q = "select distinct hour(datetime) as datetime, peak, offpeak from cust_kwh_logs where cid = '$id' and datetime in (select max(datetime) from cust_kwh_logs where cid ='$id' and datetime > DATE_SUB(now(), INTERVAL 25 hour) group by hour(datetime))";
